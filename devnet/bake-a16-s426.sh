#!/usr/bin/env bash
# bake-a16-s426.sh — A1.6 large-state devnet bake for the incremental state root.
#
...
#     scan every block). Artifacts under devnet/bake-a16-s426/stage-N/.
#
...
#
# Syntax gate: bash -n devnet/bake-a16-s426.sh
set -uo pipefail
cd "$(dirname "$0")"
...
if [ "$SMOKE" = 1 ]; then
    ...
    BURN=${BURN:-30}
    M_LEGS=${M_LEGS:-"M2"}
    OUTROOT=${OUTROOT:-bake-a16-s426-smoke}
else
