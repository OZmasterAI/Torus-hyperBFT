...
pub struct SharedState {
    ...
    /// worker. Installed once at startup; an empty cell (flag off, tests,
    /// observers) costs a presence check but no lock acquisition or datum copy.
    pub(crate) validate_tee: OnceLock<Arc<dyn Fn(Vec<u8>) + Send + Sync>>,
}
...
fn enqueue_inbound(
    ...
    // calls `validate_block` a heavy verdict may already be cached. Experimental:
...
/// move the work back onto the thread being relieved. When no tee is installed
/// (flag off), only a `OnceLock::get` presence check runs, before datum extraction.
fn tee_proposal_datum(shared: &SharedState, msg: &hotstuff_rs::networking::messages::Message) {
    use hotstuff_rs::hotstuff::messages::HotStuffMessage;
    use hotstuff_rs::networking::messages::{Message, ProgressMessage};
    let Some(tee) = shared.validate_tee.get() else { return };
    let block = match msg {
...
mod tests {
    ...
    fn validate_tee_receives_proposal_datum_on_enqueue() {
        ...
        assert!(shared
            .validate_tee
            .set(Arc::new(move |bytes: Vec<u8>| sink.lock().unwrap().push(bytes)))
            .is_ok());
    ...
    /// Flag OFF leaves the hook empty and delivers proposal and non-proposal
    /// messages, preserving sender, payload, and FIFO order.
    #[test]
    fn validate_tee_uninstalled_preserves_enqueue_delivery() {
        use hotstuff_rs::hotstuff::messages::{HotStuffMessage, Proposal};
        use hotstuff_rs::hotstuff::types::PhaseCertificate;
        use hotstuff_rs::types::block::Block;
        use hotstuff_rs::types::data_types::{
        ...
        let shared = test_shared();
        assert!(shared.validate_tee.get().is_none());
        let proposal: hotstuff_rs::networking::messages::Message =
            HotStuffMessage::Proposal(Proposal {
                ...
                block: Block::new(
                    ...
                    PhaseCertificate::genesis_pc(),
                    CryptoHash::new([0u8; 32]),
                    Data::new(vec![Datum::new(vec![7u8, 8, 9])]),
                ),
                tc: None,
                nec: None,
            })
            .into();
        let advance_view: hotstuff_rs::networking::messages::Message =
            hotstuff_rs::pacemaker::messages::PacemakerMessage::AdvanceView(
                hotstuff_rs::pacemaker::messages::AdvanceView {
                    progress_certificate: PhaseCertificate::genesis_pc().into(),
                },
            ...
            .into();
        let expected = [
            (test_vk(1), proposal.try_to_vec().unwrap()),
            (test_vk(2), advance_view.try_to_vec().unwrap()),
        ];
        assert!(enqueue_inbound(&shared, test_vk(1), proposal));
        assert!(enqueue_inbound(&shared, test_vk(2), advance_view));
        let mut queue = shared.inbound.lock().unwrap();
        for (sender, bytes) in expected {
            let (got_sender, got_msg) = queue.pop_front().expect("message delivered");
            assert_eq!(got_sender, sender);
            assert_eq!(got_msg.try_to_vec().unwrap(), bytes);
        }
