...
#[derive(Clone)]
pub(crate) struct MemSnap(HashMap<Vec<u8>, Vec<u8>>);
...
pub(crate) fn signing_keys(seeds: &[u8]) -> Vec<SigningKey> {
    seeds
        .iter()
...
pub(crate) fn steady_block_tree(set: &ValidatorSet) -> (BlockTreeSingleton<MemKV>, ValidatorSetState) {
...
pub(crate) fn proposer_for<K: KVStore>(
    view: ViewNumber,
    keys: &[SigningKey],
