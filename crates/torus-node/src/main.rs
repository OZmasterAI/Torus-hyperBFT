...
async fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // the hotstuff-algo thread finds a ready heavy-stage verdict instead of doing
    // the DA reconstruct + sig verify inline. Wired AFTER set_native_da_fetcher,
    // which re-spawns the worker (earlier handles would go stale). Flag off (the
    // default): no handle exists and nothing is installed — byte-identical.
