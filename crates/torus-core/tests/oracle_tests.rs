...
fn outlier_rejected() {
    ...
    mgr.submit_price(&addr(1), market, fp(990), block, 0).unwrap();
    mgr.submit_price(&addr(2), market, fp(1000), block, 0).unwrap();
    mgr.submit_price(&addr(3), market, fp(1010), block, 0).unwrap();
    mgr.submit_price(&addr(4), market, fp(1005), block, 0).unwrap();
    mgr.submit_price(&addr(5), market, fp(50000), block, 0).unwrap();

    let stakes: Vec<(Address, FixedPoint)> = (1..=5)
        .map(|i| (addr(i), fp(10)))
        .collect();
...
fn staleness_detection() {
    ...
    // Submit at block 100
    mgr.submit_price(&addr(1), market, fp(1000), 100, 0).unwrap();
...
fn single_validator_works() {
    ...
    mgr.submit_price(&addr(1), market, fp(5000), 100, 0).unwrap();
...
fn all_same_price() {
    ...
    for i in 1..=5u8 {
        mgr.submit_price(&addr(i), market, fp(2500), 100, 0).unwrap();
    }
...
fn validator_overwrites_same_block() {
    ...
    mgr.submit_price(&addr(1), market, fp(1000), 100, 0).unwrap();
    mgr.submit_price(&addr(1), market, fp(2000), 100, 0).unwrap(); // overwrite
...
fn no_floating_point_in_aggregation() {
    ...
    mgr.submit_price(&addr(1), market, FixedPoint::from_raw(333_333_333), 100, 0).unwrap();
    mgr.submit_price(&addr(2), market, FixedPoint::from_raw(333_333_334), 100, 0).unwrap();
    mgr.submit_price(&addr(3), market, FixedPoint::from_raw(333_333_335), 100, 0).unwrap();
...
fn oracle_price_reports_num_reporters() {
    ...
    for i in 1..=5u8 {
        mgr.submit_price(&addr(i), market, fp(1000), 100, 0).unwrap();
    }
