...
use torus_types::{
    ActionSignature, Ed25519Sig, NativeAction, SessionData, SessionScope, SignedNativeAction,
    FixedPoint, OrderType, PlaceOrderParams, TimeInForce,
};
...
fn session_key_place_order_resolves_to_owner() {
    ...
    // Resolve sender
    let resolved = signed.resolve_sender(timestamp, |pk| {
        state_db.get_session(pk).ok().flatten()
    });
...
fn session_key_withdraw_rejected_scope_violation() {
    ...
    let result = signed.resolve_sender(timestamp, |pk| {
        state_db.get_session(pk).ok().flatten()
    });
    assert_eq!(result.unwrap_err(), torus_types::eip712::Eip712Error::RequiresEip712);
}
...
fn expired_session_key_rejected() {
    ...
    let result = signed.resolve_sender(current_time, |pk| {
        state_db.get_session(pk).ok().flatten()
    });
    assert_eq!(result.unwrap_err(), torus_types::eip712::Eip712Error::SessionExpired);
}
...
fn revoke_session_then_use_fails() {
    ...
    let revoke_action = NativeAction::RevokeSession { session_pubkey: pubkey };
    ...
    assert_eq!(result.unwrap_err(), torus_types::eip712::Eip712Error::SessionNotFound);
}
...
fn invalid_ed25519_signature_rejected() {
    ...
    assert_eq!(result.unwrap_err(), torus_types::eip712::Eip712Error::SessionSignatureInvalid);
}
...
fn session_scope_transfers_only_blocks_trading() {
    ...
    let transfer = NativeAction::TransferToPerp { amount: U256::from(100) };
    ...
    assert_eq!(result.unwrap_err(), torus_types::eip712::Eip712Error::SessionScopeViolation);
}
