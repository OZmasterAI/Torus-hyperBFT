    ...
    pub fn save_order_books(&mut self) -> usize {
        ...
        for market_id in dirty {
            ...
            match self.book_mode {
                ...
                BookMode::LevelAuthority | BookMode::LevelAuthorityChunked => {
                    if self.book_mode.level_hash_chunked() {
                        // Mode 3: chunked digest (idempotent select; the load
                        // path already set it for rebuilt books — this covers
                        // books created since, e.g. a market's first order).
                        book.set_level_hash_chunked(true);
                        book.disable_level_hash_cache();
                    } else if level_cache_per_book > 0 {
                        // L3: enable/refresh (or actively drop, when budget =
                        // 0) the level-hash sponge cache before draining level
                        // ops. Byte-identical output in both states.
