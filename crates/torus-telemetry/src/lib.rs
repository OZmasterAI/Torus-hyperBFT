...
pub struct Metrics {
    ...
    /// L3 flush-pipe (b): per-block block-body batches queued to the post-flush
    ...
    /// r3: consensus-thread commit-time durable persist (`FIX 1a`: header +
    /// body + manifest prune, ONE WriteBatch) — the biggest single write on the
    /// HotStuff thread and the mirror of the exec-side small-write wait.
...
impl Metrics {
    pub fn new() -> Self {
        ...
        let post_flush_writer_queued_batches = Gauge::default();
        registry.register(
            "torus_post_flush_writer_queued_batches",
            "Block-body KV batches queued to the post-flush background writer but not yet written",
