...
pub struct Metrics {
    ...
    /// Native-action pre-spread batches whose gossipsub `publish` returned an
    /// error (RH3 / finding #5), counted by the number of actions in the failed
    /// batch. The batch is already cleared before publish, so without this a
    /// dropped pre-spread batch vanished silently (bodies still reach inclusion
    /// via the pre-proposal push / DA pull path).
