...
/// disk already. Produced by [`persist_committed_block_durably`] (both `true`
/// only when its single WriteBatch returned `Ok`) and carried to the exec thread
/// so it can skip the redundant exec-time rewrite. Neither CF is part of any
/// consensus root: this is a pure node-local I/O saving, never a state /
/// determinism change.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
struct DurableRows {
    /// `CF_BLOCK_HEADERS[height]` (hash ‖ header JSON) is durable.
    header: bool,
    /// `CF_BLOCK_BODIES[height]` (body record, see `torus_state::block_body`) is durable.
    body: bool,
}
...
/// arrival on the NETWORK thread. Anything else (INCLUDING UNSET) uses synchronous
/// validation without a worker; equivalent behavior does not imply identical cost.
/// Flag ON is experimental: pre-consensus-check admission/cache churn, speculative
/// durable-write amplification, and tee-race duplicate metrics remain gate items.
/// Resolved ONCE at `TorusApp::new` and logged there (`log_async_validate_resolution`).
...
struct ExecutionContext {
    ...
    /// Deferred book save (s63 port of item 6a): whether `last_job` is a
    /// `Job::Flush` whose book-save pass 2 rides W's sidecar — i.e. its book
    /// bytes are NOT in the parent layer. A next block that would REBUILD its
    /// books (resident holder not the predecessor) must then barrier first,
    /// so the reload reads them from the DB. Cleared by every barrier.
    last_job_books_deferred: AtomicBool,
    /// Test-only stand-in for the process-global `TORUS_BOOK_ROWS` +
    /// `TORUS_RESIDENT_BOOKS=1` (`OnceLock`s in production): `Some(mode)` builds
...
/// loss).
///
/// r4 commit-persist (consensus critical path):
/// * The body is stored as a `torus_state::block_body` record — by default the
///   tagged bin (bincode wire) encoding written straight from the borrowed
///   block (no deep clone of the 25–80k `PlaceOrderParams` in a full block, no
///   multi-MB JSON text on the HotStuff thread). Every reader decodes both the
///   legacy JSON and the tagged bin record.
/// * Header + body + the commit-manifest prune (the manifest is redundant once
///   the body is durable) ride ONE atomic `WriteBatch` — one RocksDB write group
///   on the consensus thread instead of three, so the HotStuff thread queues
///   behind concurrent writers (the exec flush, the background trade writer)
///   once per commit, not three times. Crash semantics are at least as strong
///   as the separate puts: header and body appear together, so a crash can
///   never leave the header (committed height visible to
///   `find_last_committed_height`) without its body; on a failed batch the
///   manifest is KEPT since the body is not durable.
/// * Returns which rows landed ([`DurableRows`]). The dispatcher forwards that
///   to the exec thread, which SKIPS its own (previously idempotent-repeat)
///   header/body rewrite for a row reported durable — the exec-time rewrite of
///   a full block body cost 66–269 ms of deep-clone + JSON encode + put on the
///   exec critical chain per block, for bytes already in the same key. A row
///   that failed here (`false`) still gets the exec-time write, so the
///   boot-replay / block-sync / RPC readers see exactly the coverage they had
///   before. The EVM arm's `commit_block_metadata` (receipts/indices, only
///   known post-execution) is untouched.
#[cfg(test)]
fn persist_committed_block_durably(state_db: &StateDb, block: &TorusBlock) -> DurableRows {
    persist_committed_block_durably_timed(state_db, block, None)
}

/// [`persist_committed_block_durably`] with the consensus-thread timers
/// (`torus_commit_persist_seconds` = whole call, `torus_commit_body_encode_seconds`
/// = the body-record encode alone, `torus_commit_persist_write_seconds` = the
/// WriteBatch write alone).
fn persist_committed_block_durably_timed(
    state_db: &StateDb,
    block: &TorusBlock,
    ...
    // Header first in the batch (find_last_committed_height scans CF_BLOCK_HEADERS):
    // header and body land atomically, so a crash between this write and
    // execution still exposes the committed height AND its body for the replay.
    match (state_db.cf_handle(CF_BLOCK_HEADERS), header_record_bytes(block)) {
        (Ok(cf), Some(header_bytes)) => {
            batch.put_cf(cf, height_key, &header_bytes);
            header = true;
        }
        (Err(e), _) => {
    ...
    let encode_timer = std::time::Instant::now();
    let body_record = torus_state::block_body::encode_body_record(block);
    if let Some(m) = metrics {
        m.commit_body_encode_seconds
            .observe(encode_timer.elapsed().as_secs_f64());
    }
    match (state_db.cf_handle(CF_BLOCK_BODIES), body_record) {
        (Ok(cf), Ok(body_bytes)) => {
            batch.put_cf(cf, height_key, &body_bytes);
            body = true;
        }
        (Err(e), _) => {
    ...
    // window. Only when the body is actually in the batch: a still-missing body
    // keeps its manifest for the heal pull. (A missing manifest CF is harmless:
    // the CF merely grows.)
    if body {
    ...
    if let Some(m) = metrics {
        m.commit_persist_write_seconds
            .observe(write_timer.elapsed().as_secs_f64());
    }
...
fn load_replay_body(state_db: &StateDb, height: u64) -> Option<TorusBlockBody> {
    match state_db.get_cf_raw(CF_BLOCK_BODIES, &height.to_be_bytes()) {
        Ok(Some(data)) => match torus_state::block_body::decode_body_record(&data) {
            Ok(b) => Some(b),
            Err(e) => {
...
impl ExecutionContext {
    ...
    fn attach_flush_worker(&mut self, gate: Option<Arc<crate::exec_pipeline::WorkerGate>>) {
        ...
        *self.last_job.lock().unwrap() = None;
        self.last_job_books_deferred.store(false, Ordering::SeqCst);
        tracing::info!(
            applied,
    ...
    fn pipeline_handoff(&self, job: crate::exec_pipeline::Job) -> bool {
        ...
        let books_deferred = matches!(
            &job,
            crate::exec_pipeline::Job::Flush { books: Some(_), .. }
        );
        ...
        *self.last_job.lock().unwrap() = Some(pending);
        self.last_job_books_deferred
            .store(books_deferred, Ordering::SeqCst);
        true
    }
    ...
    fn execute_committed_block_with(
        ...
        if has_native || computed_fee_revenue > 0 {
            ...
            // block. Attribute it explicitly (near-zero when resident reuse hits).
            // Deferred book save hazard guard: the parent job's book bytes ride
            // W's sidecar, NOT the parent layer. If this block would rebuild its
            // books (holder is not the predecessor's post-state — drained or
            // skipped; unreachable in the normal sequence), wait for W first so
            // the reload reads them from the DB. Resident reuse (the normal
            // case) never reads persisted books, so it never waits here.
            if pipelined
                && self.last_job_books_deferred.load(Ordering::SeqCst)
                && resident_books.height() != height.checked_sub(1)
            {
                tracing::warn!(
                    height,
                    resident_height = ?resident_books.height(),
                    "deferred book save: resident books will rebuild after a deferred parent — \
                     waiting for the flush worker first"
                );
                if !self.pipeline_barrier() {
                    tracing::error!(
                        height,
                        "FATAL: flush worker failed before resident-book rebuild barrier — fail-stop"
                    );
                    return;
                }
            }
            ...
            // pipelined + resident books + level-authority mode (2/3) → pass 1
            // (drain) runs here, pass 2 (the state writes) rides Job::Flush to W,
            // joining W's one atomic batch and the root computation (see
            // exec_pipeline.rs / native_executor.rs). Every other combination is
            // exactly the serial save. Resident gating: without resident books
            // the NEXT block reloads books through this overlay's parent layer,
            // which must then already hold the book bytes — those nodes keep the
            // on-thread pass 2. `save_order_books_deferred` returns None (no side
            // effects) for Classic / OrderRows.
            ...
            if let Some(ref m) = self.metrics {
                ...
                // Deferred blocks: the write half runs on W, which observes
                // `exec_save_books_write_seconds` when it applies the sidecar —
                // still ONE observation per native block.
                if deferred_books.is_none() {
        ...
        //
        // r4 commit-persist: when the dispatcher already landed this exact body
        // (`durable.body`, `persist_committed_block_durably` at commit time), the
        // deep clone + encode + put below would put byte-identical bytes at the
        // same key — 66–269 ms per full block on the exec critical chain, plus MBs
        // of WAL/memtable per block per validator. Skip it. Rows NOT reported
        // durable (boot crash-replay, a failed dispatch batch,
        // `TORUS_EXEC_REWRITE_DURABLE_ROWS=1`) are still written here — as the
        // same `torus_state::block_body` record the dispatcher would have written
        // (borrowed encode, no clone) — so the crash-replay / block-sync / RPC
        // readers keep exactly their old coverage.
        let body_persist_timer = std::time::Instant::now();
        let body_bytes = if durable.body {
            None
        } else {
            torus_state::block_body::encode_body_record(torus_block).ok()
        };
        ...
        if !(has_native || computed_fee_revenue > 0) {
            ...
            // failed handoff returned early — the holder is NOT advanced past a
            // marker that will never land.
            self.resident_books
                .lock()
                .expect("resident-books mutex poisoned (exec thread panicked mid-block)")
...
pub struct TorusApp {
    ...
    /// validate path then runs synchronously; cost parity still needs measurement.
            ...
            .spawn(move || {
                while let Ok(datum_bytes) = rx.recv() {
                    ...
                    let block = match TorusApp::decode_proposal_and_ensure_durable_core(
                        ...
                        &datum_bytes,
                        |height, hashes| notes.push((height, hashes.to_vec())),
                    ) {
                        Ok(block) => block,
...
impl TorusApp {
    ...
    fn decode_proposal_and_ensure_durable(
        ...
        let in_flight_hashes = &mut self.in_flight_hashes;
        Self::decode_proposal_and_ensure_durable_core(
            self.mempool.as_ref(),
            self.da_fetcher.as_ref(),
            self.da_recovery.as_ref(),
            &self.metrics,
            self.last_validator_set.validators.len(),
            datum_bytes,
            |height, hashes| in_flight_hashes.note(height, hashes.iter().copied()),
        )
    }

    /// Static core of `decode_proposal_and_ensure_durable`, shared with the
    /// `torus-async-validate` worker. The callback borrows compact-path hashes
    /// BEFORE reconstruction, including on MissingData. The sync caller updates
    /// its ledger directly; only the worker buffers owned notes for later use.
    #[allow(clippy::too_many_arguments)]
    fn decode_proposal_and_ensure_durable_core(
        ...
        validator_count: usize,
        datum_bytes: &[u8],
        mut note_in_flight: impl FnMut(u64, &[torus_types::B256]),
    ) -> Result<TorusBlock, ValidateBlockResponse> {
        ...
        if let Some(block) = full {
            ...
            note_in_flight(compact.header.height, &compact.native_action_hashes);
    ...
    /// Test entry point without the crate-private `ValidateBlockRequest`.
    /// Production checks fail-stop once in `validate_block`, BEFORE structure.
    #[cfg(test)]
    fn validate_datum(
        &mut self,
        datum_bytes: &[u8],
...
impl TorusApp {
    ...
    fn dispatch_to_exec(
        ...
        if self.exec_prepared_height != Some(height) {
            ...
            // authoritative.
            // The rows it lands are reported to the exec thread, which SKIPS its own
            // (formerly idempotent-repeat) header/body rewrite for them; a row that
            // failed here is still rewritten at exec, as before.
            // `TORUS_EXEC_REWRITE_DURABLE_ROWS=1` forces the pre-skip rewrite (bisect).
            // The commit manifest that guarded the heal channel for this height is
            // pruned INSIDE the same WriteBatch (redundant once the body is durable),
            // keeping `CF_COMMIT_MANIFEST` bounded to the committed-but-not-yet-
            // dispatched window; a still-parked hole never reaches here, so its
            // manifest is retained for the pull.
            let durable = persist_committed_block_durably_timed(
                &self.state_db,
                &torus_block,
                self.metrics.as_deref(),
            );
...
mod crash_recovery_tests {
    ...
    /// r4 commit-persist RED-first: the commit-time persist writes the body as a
    /// TAGGED BIN record (`torus_state::block_body`, tag 0x01 + bincode wire
    /// encoding — no JSON text, no clone), prunes the height's commit manifest in
    /// the SAME WriteBatch, reports both rows durable, and the crash-replay reader
    /// decodes the record. RED on pre-r4 code: the body row was JSON (`{`), the
    /// manifest was pruned by a separate delete, and the fn returned `()`.
    #[test]
    fn commit_persist_writes_tagged_bin_record_and_prunes_manifest_atomically() {
        let (_config, state_db) = make_test_config_and_db();
        let block = make_block(9, vec![sign_claim_rewards(9), sign_claim_rewards(9)]);
        let key = 9u64.to_be_bytes();
        persist_commit_manifest(&state_db, 9, b"datum-9");
        assert!(state_db.get_cf_raw(CF_COMMIT_MANIFEST, &key).unwrap().is_some());
        ...
        assert_eq!(
            ...
            "both rows land in the single WriteBatch"
        );

        let raw_body = state_db.get_cf_raw(CF_BLOCK_BODIES, &key).unwrap().unwrap();
        assert_eq!(
            raw_body[0],
            torus_state::block_body::BODY_RECORD_TAG_BIN_V1,
            "default body record is the tagged bin (wire) encoding, not JSON"
        );
        assert_eq!(
            &raw_body[1..],
            &bincode::serialize(&block.body()).unwrap()[..],
            "payload after the tag IS the bincode wire encoding of the body"
        );
        assert!(
            state_db.get_cf_raw(CF_COMMIT_MANIFEST, &key).unwrap().is_none(),
            "commit manifest pruned in the same batch as the durable body"
        );
        let b = load_replay_body(&state_db, 9).expect("crash-replay reader decodes the bin record");
        assert_eq!(b.native_actions.len(), 2);
        assert_eq!(
            torus_types::compute_action_hash(&b.native_actions[1]),
            torus_types::compute_action_hash(&block.native_actions[1]),
            "actions round-trip hash-for-hash"
        );
        assert_eq!(load_replay_header(&state_db, 9).unwrap().native_action_count, 2);
    }

    /// r4 commit-persist: crash-replay must decode BOTH record formats — heights
    /// written before the upgrade (legacy JSON) and after (tagged bin) — and the
    /// two must yield the same actions for the same block.
    #[test]
    fn crash_replay_reads_legacy_json_and_tagged_bin_bodies() {
        let (_config, state_db) = make_test_config_and_db();
        let b2 = make_block(2, vec![sign_claim_rewards(2)]);
        let b3 = make_block(3, vec![sign_claim_rewards(3), sign_claim_rewards(33)]);
        persist_block_for_test(&state_db, &b2); // legacy JSON writer
        persist_committed_block_durably(&state_db, &b3); // tagged bin writer
        assert_eq!(
            state_db.get_cf_raw(CF_BLOCK_BODIES, &2u64.to_be_bytes()).unwrap().unwrap()[0],
            b'{'
        );
        assert_eq!(
            state_db.get_cf_raw(CF_BLOCK_BODIES, &3u64.to_be_bytes()).unwrap().unwrap()[0],
            torus_state::block_body::BODY_RECORD_TAG_BIN_V1
        );
        let r2 = load_replay_body(&state_db, 2).expect("legacy JSON body decodes");
        let r3 = load_replay_body(&state_db, 3).expect("tagged bin body decodes");
        assert_eq!(r2.native_actions.len(), 1);
        assert_eq!(r3.native_actions.len(), 2);
        let hashes = |v: &[SignedNativeAction]| -> Vec<torus_types::B256> {
            v.iter().map(torus_types::compute_action_hash).collect()
        };
        assert_eq!(hashes(&r2.native_actions), hashes(&b2.native_actions));
        assert_eq!(hashes(&r3.native_actions), hashes(&b3.native_actions));

        // And the same block written in both formats decodes to the same actions.
        let legacy = torus_state::block_body::encode_body_record_as(
            &b3,
            torus_state::block_body::BodyRecordFormat::LegacyJson,
        )
        .unwrap();
        state_db.put_cf_raw(CF_BLOCK_BODIES, &3u64.to_be_bytes(), &legacy).unwrap();
        let r3_legacy = load_replay_body(&state_db, 3).unwrap();
        assert_eq!(hashes(&r3_legacy.native_actions), hashes(&r3.native_actions));
    }

    /// r4 commit-persist / body-persist-skip RED-first: a NON-EMPTY native block
    /// whose header+body the dispatcher already made durable
    /// (`persist_committed_block_durably` ⇒ both rows `true`) must NOT be
    /// re-encoded / re-written by execution. Proven by tagging both durable rows
    /// with a trailing sentinel BEFORE exec: a rewrite would replace the row
    /// (sentinel gone); the skip leaves the tagged bytes intact. Execution itself
    /// is unaffected (applied advances, no fail-stop).
    #[test]
    ...
    /// path, or a dispatch whose batch failed) still gets both exec-time writes
    /// (the body as the tagged bin record), and the body row round-trips through
    /// the crash-replay reader. Partial flags are honored per row.
    #[test]
    fn exec_rewrites_rows_not_reported_durable() {
        ...
        let raw = state_db.get_cf_raw(CF_BLOCK_BODIES, &key).unwrap().unwrap();
        assert_eq!(raw[0], torus_state::block_body::BODY_RECORD_TAG_BIN_V1);
        ...
        // (b) header durable, body NOT (a failed dispatch body write): exec rewrites
        // only the body row.
    ...
    #[test]
    fn pending_proposal_warm_and_cold_match_reference_for_body_variants() {
        let actions = vec![sign_claim_rewards(1), big_order_batch_action(2, 400)];
        let mut resigned = actions.clone();
        let torus_types::ActionSignature::Eip712(ref mut signature) = resigned[0].signature
        else {
            panic!("fixture must have an EIP-712 signature");
        };
        signature.r[0] ^= 1;
        let bodies = vec![
            ...
            vec![actions[0].clone(), actions[0].clone()],
            vec![sign_claim_rewards(3), actions[1].clone()],
            resigned,
        ];
        for body in &bodies {
            let block = make_block(7, body.clone());
            let cold = PendingProposal::new(block.clone());
            let warm = PendingProposal::new(block.clone());
            let addresses = warm.native_action_hashes().as_ptr();
            for references in &bodies {
                let compact = CompactBlock::from_block(&make_block(7, references.clone()));
                // Original streaming predicate is the independent oracle; it
                // binds length, order, duplicates, content and signatures.
                let expected = block.native_actions.len() == compact.native_action_hashes.len()
                    && block
                        .native_actions
                        .iter()
                        .map(torus_types::compute_action_hash)
                        .eq(compact.native_action_hashes.iter().copied());
                assert_eq!(cached_matches_compact(&cold, &compact), expected);
                assert_eq!(cached_matches_compact(&warm, &compact), expected);
                assert!(cold.native_action_hashes.get().is_none());
                assert_eq!(warm.native_action_hashes().as_ptr(), addresses);
            }
        }
    }

    #[test]
    fn pending_proposal_replacement_discards_previous_hashes() {
        let mut app = TorusApp::stub();
        ...
        assert!(pending.native_action_hashes.get().is_none());
        assert_ne!(pending.native_action_hashes(), original_hashes.as_slice());
        assert!(!cached_matches_compact(
            pending,
            &CompactBlock::from_block(&original)
        ));
        assert!(cached_matches_compact(
            pending,
            &CompactBlock::from_block(&replacement)
        ));
    }

    #[test]
    fn pending_proposal_warm_commit_uses_committed_header_and_inline_payload() {
        let (config, state_db) = make_test_config_and_db();
        let (mut app, rx, _mempool) = app_with_recording_exec(&config, &state_db);
        app.exec_next_height = Some(5);
        let mut cached = make_block(5, vec![sign_claim_rewards(1)]);
        cached.header.timestamp = 10;
        cached.header.evm_tx_count = 1;
        // Opaque EVM bytes are sufficient: this test records dispatch and does
        // not bypass or call the independent validation/execution stages.
        cached.evm_transactions = vec![vec![0xaa]];
        let mut committed = cached.clone();
        committed.header.timestamp = 20;
        committed.header.base_fee_per_gas += 1;
        committed.evm_transactions = vec![vec![0xbb, 0xcc]];
        committed.core_writer_actions = vec![torus_types::CoreWriterAction::PermanentStake {
            amount: U256::from(42),
        }];
        let pending = PendingProposal::new(cached);
        pending.native_action_hashes();
        app.pending_proposals.insert(5, pending);
        let consensus_block = committed_compact_block(&committed);
        app.on_committed_block(&consensus_block, consensus_block.hash);
        let msg = rx.try_recv().expect("matching warm cache must dispatch");
        assert_eq!(
            bincode::serialize(&msg.torus_block).unwrap(),
            bincode::serialize(&committed).unwrap()
        );
        assert_eq!(
            msg.durable,
            DurableRows { header: true, body: true }
        );
        let durable_body = load_replay_body(&state_db, 5).expect("body durable before dispatch");
        assert_eq!(
            bincode::serialize(&durable_body).unwrap(),
            bincode::serialize(&committed.body()).unwrap()
        );
        assert!(!app.pending_proposals.contains_key(&5));
    }
    ...
    // uses synchronous validation; cache misses/stale verdicts fall back to it.
    // Flag ON remains experimental pending the admission/write/metrics gates.
    ...
    #[test]
    fn compact_decode_borrowed_notes_survive_missing_data() {
        let metrics = Some(Arc::new(torus_telemetry::Metrics::new()));
        let mut ledger = InFlightHashLedger::default();
        let mut calls = 0;
        // Same-height re-proposals must union notes even when neither body can
        // be reconstructed. No mempool means reconstruction fails immediately.
        for nonce in [1, 2] {
            let block = make_block(5, vec![sign_claim_rewards(nonce)]);
            let expected = torus_types::compute_action_hash(&block.native_actions[0]);
            let datum = encode_proposal_datum(&block, true);
            let response = TorusApp::decode_proposal_and_ensure_durable_core(
                None,
                None,
                None,
                &metrics,
                ...
                |height, hashes: &[B256]| {
                    ...
                    assert_eq!(
                        ...
                        calls,
                        "callback must precede this proposal's reconstruction"
                    );
                    ledger.note(height, hashes.iter().copied());
                    calls += 1;
                },
            );
            assert!(matches!(response, Err(ValidateBlockResponse::MissingData)));
            assert!(ledger.by_height[&5].contains(&expected));
        }
        assert_eq!(calls, 2);
        assert_eq!(ledger.by_height[&5].len(), 2);
    }

    #[test]
    fn compact_decode_note_callback_runs_before_body_lookup() {
        let (_config, state_db) = make_test_config_and_db();
        let mempool = Arc::new(Mempool::new(
            state_db,
            torus_mempool::MempoolConfig::default(),
        ));
        let block = make_block(5, vec![sign_claim_rewards(3)]);
        let hash = torus_types::compute_action_hash(&block.native_actions[0]);
        let datum = encode_proposal_datum(&block, true);
        let mut calls = 0;
        let decoded = TorusApp::decode_proposal_and_ensure_durable_core(
            Some(&mempool),
            None,
            None,
            &None,
            ...
            |height, hashes| {
                assert_eq!(height, 5);
                assert_eq!(hashes, &[hash]);
                // The callback supplies the body: reconstruction succeeds only
                // if it runs AFTER this borrowed note is delivered.
                mempool.mirror_native_to_da(&block.native_actions).unwrap();
                calls += 1;
            },
        );
        assert_eq!(calls, 1);
        let decoded = match decoded {
            Ok(block) => block,
            Err(_) => panic!("callback must run before body lookup"),
        };
        assert_eq!(
    ...
    fn async_validate_flag_off_parity() {
        ...
        assert!(
            app.async_validate_handle().is_none(),
            "OFF exposes no worker feed"
        );
        ...
        // A reconstruction miss still updates the sync ledger immediately.
        let missing = make_block(6, vec![sign_claim_rewards(3)]);
        let missing_hash = torus_types::compute_action_hash(&missing.native_actions[0]);
        ...
        assert!(app.in_flight_hashes.by_height[&6].contains(&missing_hash));
        assert!(!app.pending_proposals.contains_key(&6));
        assert!(
            app.async_validate.is_none(),
            "validation must not lazily spawn a worker"
        );
        assert!(app.async_validate_handle().is_none());

        // Direct test callers retain fail-stop, before any decode/body work.
    ...
    /// r4 commit-persist: the live dispatch path (`on_committed_block` →
    ...
    fn dispatch_reports_durable_rows_after_persisting_them() {
        ...
        assert!(
            state_db.get_cf_raw(CF_COMMIT_MANIFEST, &key).unwrap().is_none(),
            "commit manifest pruned at dispatch (same batch as the durable body)"
        );
    }
    ...
    // ======================================================================
    // s63 port of item 6a (origin 4298728): book-save pass 2 deferred to W
    // ======================================================================

    /// The deferred mode-2 book save (`save_order_books_deferred`) rides
    /// `Job::Flush` to the REAL flush worker, which applies it as a sidecar of
    /// the block's one atomic batch. Pins, against a serial reference over the
    /// same blocks:
    ...
    fn exec_pipeline_deferred_books_atomic_and_match_serial() {
        ...
        for h in [1u64, 2] {
            let (frozen, save) =
                exec_block(&db_w, &mut books_w, h, &blocks[(h - 1) as usize], true);
    ...
    /// Book-heavy sequence for the END-TO-END deferred-save tests (through
    /// `execute_committed_block_with`, epoch_length 4 → 4/8/12 are serial
    /// barriers). Covers resting sprays over several levels, a stop order,
    /// partial fills, a multi-level sweep, cancel-alls and empties:
    ///   1  fund A,B                                             native
    ///   2  A sells 100x3, 101x2, 102x1; B buys 99x1 + stop      native (deferred)
    ///   3  B buy 100x1 -> PARTIAL fill of A's 100x3            native (deferred, reads 2's books)
    ///   4  empty (epoch boundary)                               serial marker
    ///   5  B buy 101x3 -> sweeps 100x2 + partial 101           native (deferred)
    ///   6  empty                                                Marker job
    ///   7  A cancel-all mkt 1; B buy 97x2                       native (deferred)
    ///   8  A sell 97x1 -> fills B's 99 bid (epoch boundary)    native, SERIAL
    ///   9  A sell 96x1 -> partial of B's 97x2 level             native (deferred)
    ///  10  B cancel-all (stop + 97 remainder)                   native (deferred)
    ///  11  A sell 105x2 resting                                 native (deferred)
    ///  12  empty (epoch boundary)                               serial marker
    ///  13  B buy 105x1 -> partial fill, 105x1 stays resting     native (deferred)
    fn book_fixture_blocks() -> Vec<TorusBlock> {
        ...
        let stop = torus_types::PlaceOrderParams {
            order_type: torus_types::OrderType::StopMarket {
                trigger: FixedPoint::from_raw(150 * FixedPoint::SCALE),
            },
            ..order(true, 100, 1)
        ...
        let mut blocks = vec![
            make_block(
                1,
                vec![
                    sign(NativeAction::TransferToPerp { amount: deposit }, 1, &k_a),
            ...
            make_block(
                2,
                vec![
                    sign(place(order(false, 100, 3)), 2, &k_a),
                    sign(place(order(false, 101, 2)), 3, &k_a),
                    sign(place(order(false, 102, 1)), 4, &k_a),
                    sign(place(order(true, 99, 1)), 2, &k_b),
                    sign(place(stop), 3, &k_b),
                ],
            ),
            make_block(3, vec![sign(place(order(true, 100, 1)), 4, &k_b)]),
            make_block(4, vec![]),
            make_block(5, vec![sign(place(order(true, 101, 3)), 5, &k_b)]),
            make_block(6, vec![]),
            make_block(
                7,
                vec![
                    sign(NativeAction::CancelAllOrders { market_id: Some(1) }, 5, &k_a),
                    sign(place(order(true, 97, 2)), 6, &k_b),
                ],
            ),
            make_block(8, vec![sign(place(order(false, 97, 1)), 6, &k_a)]),
            make_block(9, vec![sign(place(order(false, 96, 1)), 7, &k_a)]),
            make_block(
                10,
                vec![sign(NativeAction::CancelAllOrders { market_id: None }, 7, &k_b)],
            ),
            make_block(11, vec![sign(place(order(false, 105, 2)), 8, &k_a)]),
            make_block(12, vec![]),
            make_block(13, vec![sign(place(order(true, 105, 1)), 8, &k_b)]),
        ];
        link_blocks(&mut blocks);
        blocks
    }

    fn fund_book_fixture(state_db: &StateDb) {
        let deposit = U256::from(1_000 * FixedPoint::ONE.raw() as u128);
        for signed in &book_fixture_blocks()[0].native_actions {
            fund_evm_balance(state_db, signed.recover_sender().unwrap(), deposit);
        }
    }

    /// `pipeline_ctx` with the book mode pinned (resident books forced on) —
    /// the test stand-in for `TORUS_BOOK_ROWS` + `TORUS_RESIDENT_BOOKS=1`,
    /// which are process-global `OnceLock`s in production.
    fn book_pipeline_ctx(
        state_db: &StateDb,
        on: bool,
        gate: Option<Arc<crate::exec_pipeline::WorkerGate>>,
    ...
    fn dump_book_cfs(db: &StateDb) -> Vec<CfDump> {
        [torus_state::cf::CF_NATIVE_ORDER_BOOKS, torus_state::cf::CF_BOOK_ORDER_ROWS]
            .iter()
            .map(|cf| (*cf, StateBackend::iterate_cf(db, cf, None).unwrap()))
            .collect()
    }

    /// Book-CF rows visible THROUGH a frozen set only (layered over an empty
    /// DB) — i.e. the book bytes E itself wrote into block N's overlay.
    fn frozen_book_rows(frozen: &Arc<torus_state::FrozenPending>) -> usize {
        let (_c, empty) = make_test_config_and_db();
        let probe = NativeStateOverlay::with_parent(empty, Some(frozen.clone()));
        [torus_state::cf::CF_NATIVE_ORDER_BOOKS, torus_state::cf::CF_BOOK_ORDER_ROWS]
            .iter()
            .map(|cf| StateBackend::iterate_cf(&probe, cf, None).unwrap().len())
            .sum()
    }
    ...
    fn run_book_fixture(
        ...
        let gate = crate::exec_pipeline::WorkerGate::new();
        let ctx = book_pipeline_ctx(&state_db, on, Some(gate.clone()), mode);
        for b in &book_fixture_blocks() {
            dispatch_and_execute(&ctx, &state_db, b);
            if on && !b.native_actions.is_empty() {
                let h = b.header.height;
                if !EpochManager::is_epoch_boundary(h, 4) {
                    // Deferral is ACTIVE: E handed W a frozen set with NO book
                    // bytes — they ride the job's sidecar instead.
                    let last = ctx.last_job.lock().unwrap().clone().expect("fast path");
                    assert_eq!(last.height(), h);
                    assert_eq!(frozen_book_rows(&last), 0, "h{h}: book writes must be deferred to W");
                }
            }
        }
        assert!(
            !ctx.exec_failed.load(std::sync::atomic::Ordering::SeqCst),
            "book fixture must not fail-stop (on={on}, {mode:?})"
        );
        drop(ctx);
        let dump = dump_all_cfs(&state_db);
        let root = torus_state::native_trie::persisted_native_root(&state_db).unwrap();
    ...
    /// END-TO-END (real `execute_committed_block_with` + real W): with the
    /// pipeline ON under level-authority modes 2 AND 3 + resident books, the
    /// deferred book save lands byte-identical state in EVERY CF and the same
    /// persisted native root as the serial (pipeline OFF) run, over a sequence
    /// with partial fills, a multi-level sweep, stops, cancel-alls, empties
    /// and serial barrier blocks interleaved. Also pins that deferral really
    /// engaged on every fast-path native block (no book bytes in E's frozen set).
    #[test]
    fn exec_pipeline_deferred_books_end_to_end_match_serial() {
        use torus_bridge::native_executor::BookMode;
        for mode in [BookMode::LevelAuthority, BookMode::LevelAuthorityChunked] {
            let (db_off, off, root_off, received_off) = run_book_fixture(false, mode);
            let (db_on, on, root_on, received_on) = run_book_fixture(true, mode);
            assert!(received_off.is_empty());
            assert_eq!(received_on, vec![1, 2, 3, 5, 6, 7, 9, 10, 11, 13], "{mode:?}");
            assert_dumps_equal(&off, &on, "deferred books ON vs OFF");
            assert_eq!(root_off, root_on, "{mode:?}: persisted native root must be identical");
            assert_eq!(read_native_applied_height(&db_on), Some(13));
            let trades =
                StateBackend::iterate_cf(&db_on, torus_state::cf::CF_NATIVE_TRADES, None).unwrap();
            assert!(trades.len() >= 5, "{mode:?}: fixture fills (not vacuous): {}", trades.len());
            let books = dump_book_cfs(&db_on);
            assert!(!books[0].1.is_empty() && !books[1].1.is_empty(), "{mode:?}: book rows persisted");
            assert_eq!(dump_book_cfs(&db_off), books);
        }
    }

    /// Crash-before-write under deferral: W's write of native 2 (with its book
    /// sidecar) fails AFTER E executed native 3 on top of pending(2). Nothing of
    /// 2/3 is durable (marker 1, no book bytes beyond block 1's), the fail-stop
    /// latches, and a serial restart-replay lands EXACTLY the serial state.
    #[test]
    fn exec_pipeline_deferred_books_crash_before_write_replays_and_matches_serial() {
        use torus_bridge::native_executor::BookMode;
        ...
        let ctx_ref = book_pipeline_ctx(&db_ref, false, None, mode);
        for b in first3 {
            dispatch_and_execute(&ctx_ref, &db_ref, b);
        }
        drop(ctx_ref);
        ...
        fund_book_fixture(&state_db);
        let gate = crate::exec_pipeline::WorkerGate::new();
        let ctx = book_pipeline_ctx(&state_db, true, Some(gate.clone()), mode);
        dispatch_and_execute(&ctx, &state_db, &first3[0]);
        assert!(ctx.flush_worker.as_ref().unwrap().wait_idle());
        assert_eq!(read_native_applied_height(&state_db), Some(1));
        let books_after_1 = dump_book_cfs(&state_db);

        gate.hold();
        dispatch_and_execute(&ctx, &state_db, &first3[1]); // Flush(2)+books received, parked
        assert!(gate.wait_received(2));
        let db_t = state_db.clone();
        let b3 = first3[2].clone();
        let t = std::thread::spawn(move || {
            dispatch_and_execute(&ctx, &db_t, &b3); // engine(3) on pending(2); hand-off blocks
            ctx
        });
        std::thread::sleep(std::time::Duration::from_millis(200));
        assert!(!t.is_finished());
        ...
        drop(ctx);
        assert_eq!(read_native_applied_height(&state_db), Some(1), "nothing of 2/3 durable");
        assert_eq!(dump_book_cfs(&state_db), books_after_1, "no book byte of 2/3 durable");

        let ctx2 = book_pipeline_ctx(&state_db, false, None, mode);
        let (_last, parked) = TorusApp::replay_committed(&state_db, &ctx2);
        assert_eq!(parked, None);
        assert!(!ctx2.exec_failed.load(std::sync::atomic::Ordering::SeqCst));
        drop(ctx2);
    ...
    /// Stale-parent guard: if the resident holder is NOT the predecessor's
    /// post-state while the parent job carried deferred book writes (holder
    /// drained), the next block would rebuild its books from the DB + a parent
    /// layer that lacks those bytes. E must instead BARRIER (wait for W) before
    /// loading — it blocks while W is parked, and the final state equals serial.
    #[test]
    fn exec_pipeline_deferred_books_rebuild_waits_for_worker() {
        use torus_bridge::native_executor::BookMode;
        let mode = BookMode::LevelAuthority;
        let blocks = book_fixture_blocks();
        let first3 = &blocks[..3];
        ...
        gate.hold();
        dispatch_and_execute(&ctx, &state_db, &first3[1]); // deferred books of 2 parked on W
        assert!(gate.wait_received(2));
        ctx.resident_books.lock().unwrap().invalidate(); // force a rebuild for 3
        let db_t = state_db.clone();
        let b3 = first3[2].clone();
        let t = std::thread::spawn(move || {
            dispatch_and_execute(&ctx, &db_t, &b3);
            ctx
        });
        std::thread::sleep(std::time::Duration::from_millis(200));
        assert!(!t.is_finished(), "E must wait for W before rebuilding books");
        gate.release();
        let ctx = t.join().unwrap();
        assert!(!ctx.exec_failed.load(std::sync::atomic::Ordering::SeqCst));
        drop(ctx);
