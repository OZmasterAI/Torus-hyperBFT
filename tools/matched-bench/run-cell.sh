...
#   (r5 root-and-save-workers sweep: TORUS_PARALLEL_BUCKET_HASH /
#   TORUS_BUCKET_MEMBER_CACHE_MB / TORUS_SAVE_BOOKS_WORKERS are plain EXTRA_ENV
#   overrides — all three are node-local and byte-neutral; the nodes publish
#   torus_exec_root_bucket_hash_workers / torus_exec_save_books_workers so the
#   summary proves the value that actually engaged. See sweep-workers.sh.)
...
for i in 0 1 2; do
    ...
    echo "ts,committed,height,placed,matched,resting,exec_resting,lb_s,lb_c,root_s,root_c,sw_s,sw_c,evm_s,evm_c,fl_s,fl_c,db_s,db_c,execq,bscan,mc_hit,mc_miss,mc_evict,mc_resident,root_workers,save_workers" > "$OUT/phase-val$i.csv"
done
sampler() {
    while true; do
        ts=$(date +%s)
